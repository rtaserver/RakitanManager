...
while true; do
    for color in "${colors[@]}"; do
        ...
        echo -e "${color}==================================================="
        echo -e "${colors[5]} Versi Terinstall: ${colors[5]}${currentVersion}  "
        echo -e "${colors[5]} Versi Terbaru: ${colors[1]}${LatestVerMain} | Branch Main | Utama"
        echo -e "${colors[5]} Versi Terbaru: ${colors[1]}${LatestVerDev} | Branch Dev | Pengembangan"
        echo -e "${color}==================================================="
        echo -e "${colors[5]} Processor: ${colors[5]}$(ubus call system board | grep '\"system\"' | sed 's/ \+/ /g' | awk -F'\"' '{print $4}')"
        echo -e "${colors[5]} Device Model: ${colors[5]}$(ubus call system board | grep '\"model\"' | sed 's/ \+/ /g' | awk -F'\"' '{print $4}')"
        echo -e "${colors[5]} Device Board: ${colors[5]}$(ubus call system board | grep '\"board_name\"' | sed 's/ \+/ /g' | awk -F'\"' '{print $4}')"
        echo -e "${color}==================================================="
        echo -e "${colors[5]} Sekedar Informasi"
        echo -e "${colors[5]}  - Branch Main : Build Yang Sudah Sekiranya Lancar"
        echo -e "${colors[5]}  - Branch Dev  : Build Yang Masih Pengembangan"
        echo -e "${colors[5]}                  Sebelum Di Alihkan Ke Branch Main"
        echo -e "${colors[5]} Maka Dari Itu Jika Ada Yang Error / Bug"
        echo -e "${colors[5]} Bisa Langsung Hubungi Saya Agar Bisa Di Perbaiki"
        echo -e "${colors[5]} Terimakasih Atas Partisipasinya :)"
        echo -e "${color}==================================================="
