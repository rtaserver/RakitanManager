...
if [ -z "$2" ]; then
	echo "DELAY is unset, using default..."
	DS="3"
else
	DS="$2"
fi
...
if [ -z "$3" ]; then
	echo "PINGCHECKER is unset, using default..."
	PINGCK="8.8.8.8"
else
	PINGCK="$3"
fi
...
if [ -z "$4" ]; then
	echo "PINGCOUNT is unset, using default..."
	PINGCT="10"
else
	PINGCT="$4"
fi
...
do
    #Ping checker system
	echo "Checking connection from device [${IPX}] to [${PINGCK}] with ping count [${PINGCT}] times...."
	httping -c ${PINGCT} ${PINGCK} > /tmp/anuping
	if grep -q "100.00% failed" /tmp/anuping; then
		echo "Network unavailable, restarting phone modem...."
	else
        #Force refresh network?
        if [ "$5" == "force" ]; then
        	echo "Network is available but Force refresh is set, restarting phone modem..."
        else
        	echo "Network is available but Force refresh is unset, leaving phone modem like default..."
	        exit 0
        fi
	fi
    ...
    echo "Airplane mode is at DISABLED state, will be enabled in ${DS} secs..."
